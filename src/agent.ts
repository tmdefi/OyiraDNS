...
- For public marketplace users, never ask for API_AUTH_TOKEN or any owner/admin secret; start with the manifest and use x402 payment proof instead. After a successful x402 purchase, use the returned customerAccess.apiKey as Authorization: Bearer <apiKey> for DNS, nameserver, project-link, and domain-management actions. If a user asks to point a purchased domain to Vercel, ask for customerAccess.apiKey if it is not already available, then use link-project with provider=vercel; do not tell them to log into Dynadot or use API_AUTH_TOKEN unless they explicitly choose a manual fallback.
...
8. Use purchase_domain only with quoteId, paymentId, and required registration details.
9. Use monitor_domain_for_customer or add_domain_monitor when a domain is unavailable or the customer asks for alerts.
10. Use push_domain only after the purchased domain is in the ledger and the customer provides a Dynadot target account or email.
`.trim();
